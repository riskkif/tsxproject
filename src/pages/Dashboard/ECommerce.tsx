...
const ECommerce: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [dosenResponse, adminResponse, userResponse, mhsResponse] = await Promise.all([
        ...
        fetch('https://stag-be.bisa.ai/api/app-admin/users?level=1', {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${token}`,
          },
        }),
        fetch('https://stag-be.bisa.ai/api/app-admin/users?level=2', {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      const adminJsonResponse = await adminResponse.json();
      const userJsonResponse = await userResponse.json();
      const mhsJsonResponse = await mhsResponse.json();
      const dosenData = dosenJsonResponse.data;
      const adminData = adminJsonResponse.data;
      const userData = userJsonResponse.data;
      const mhsData = mhsJsonResponse.data;
      ...
      if (Array.isArray(adminData)) {
        ...
        throw new Error('Data admin is not an array');
      }
      ...
      if (Array.isArray(mhsData)) {
        ...
        throw new Error('Data mahasiswa is not an array');
      }
    } catch (error) {
  ...
  const totalDosen = dosenData.length.toString();
  const totalAdmin = adminData.length.toString();
  const totalUser = userData.length.toString();
  const totalMhs = mhsData.length.toString();
  return (
    <>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        <CardDataStats title="Total Dosen" total={totalDosen} rate="0.43%" levelUp>
        <svg
        ...
        <CardDataStats title="Total Admin" total={totalAdmin} rate="4.35%" levelUp>
        <svg
            className="fill-primary dark:fill-white"
            width="22"
            height="18"
        ...
        <CardDataStats title="Total Users" total={totalUser} rate="2.59%" levelUp>
        <svg
            className="fill-primary dark:fill-white"
            width="22"
            height="18"
        ...
        <CardDataStats title="Total Mahasiswa" total={totalMhs} rate="0.95%" levelDown>
          <svg
            className="fill-primary dark:fill-white"
            width="22"
            height="18"
