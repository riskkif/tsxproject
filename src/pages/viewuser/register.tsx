...
const Registration = () => {
  ...
  const handleSubmit = async (e: { preventDefault: () => void; }) => {
    ...
    try {
      const response = await fetch('https://stag-be.bisa.ai/api/app-user/registration', {
        method: 'POST',
        body: data
      });
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-gray-600 text-center">Sudah punya akun? <Link to="/login" className='text-indigo-600 font-medium'>Login</Link></p>
            <p></p>
          </div>
        </form>
      </div>
